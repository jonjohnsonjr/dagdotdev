...
func (p *purl) url(repo string) (string, error) {
	...
	case "apk":
		if p.namespace == "alpine" {
			...
			return fmt.Sprintf("/https/dl-cdn.alpinelinux.org/alpine/edge/main/%s/%s-%s.apk", arch, p.name, p.version), nil
		} else if p.namespace == "wolfi" {
			arch := p.qualifiers.Get("arch")
			return fmt.Sprintf("/https/packages.wolfi.dev/os/%s/%s-%s.apk", arch, p.name, p.version), nil
		}
	}
