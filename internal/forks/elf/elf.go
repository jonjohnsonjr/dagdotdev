...
// 1. We have nothing cached.
// 2. We have Sections and Progs cached.
// 3. We have enough information to render in a seeky way.
var cached = &sync.Map{}
...
func Peek(r io.Reader) (*bufio.Reader, [16]uint8, bool, error) {
	...
	b, err := pr.Peek(16)
	if err != nil {
		return pr, ident, false, err
	}

	if _, err := bytes.NewReader(b).Read(ident[0:]); err != nil {
		return pr, ident, false, err
	}

	elfy := ident[0] == '\x7f' && ident[1] == 'E' && ident[2] == 'L' && ident[3] == 'F'
	return pr, ident, elfy, nil
}
...
func thirdPass(w io.Writer, size int64, r io.Reader, key string, f *File) error {
	...
	fmt.Fprintf(w, "Program Header:\n")
	headers := make([]string, 0, len(f.Progs))
	maxlen := 0
	for _, prog := range f.Progs {
		hdr := strings.TrimPrefix(strings.TrimPrefix(prog.ProgHeader.Type.String(), "PT_"), "GNU_")
		headers = append(headers, hdr)
		maxlen = max(maxlen, len(hdr))
	}

	for i, prog := range f.Progs {
		ph := prog.ProgHeader
		hdr := headers[i]
		align := int(math.Log2(float64(ph.Align)))
		fmt.Fprintf(w, "%*s", maxlen, hdr)
		fmt.Fprintf(w, " off    0x%016x vaddr 0x%016x paddr 0x%016x align 2**%d\n", ph.Off, ph.Vaddr, ph.Paddr, align)
		fmt.Fprintf(w, "%*s filesz 0x%016x memsz 0x%016x flags %s\n", maxlen, "", ph.Filesz, ph.Memsz, ph.Flags.ElfString())
	}

	fmt.Fprintf(w, "\nDynamic Section:\n")

	maxlen = 0
	for _, ds := range f.dynamicSection {
		s := strings.TrimPrefix(ds.Tag.String(), "DT_")
	...
	*/
	...
	for _, sec := range f.Sections {
		maxlen = max(maxlen, len(sec.Name))
	}
	fmt.Fprintf(w, "Idx %-*s Size     VMA              Type\n", maxlen, "Name")
	...
	for _, sym := range f.symbols {
		section := "*ABS*"
		if sym.Section < SectionIndex(len(f.Sections)) {
		...
		vis := ""
		if ST_VISIBILITY(sym.Other) == STV_HIDDEN {
			vis = ".hidden "
		} else if ST_VISIBILITY(sym.Other) == STV_PROTECTED {
			vis = ".protected "
		}
		...
		info := sym.Info
		if section != "*UND*" {
			global := ST_BIND(info) == STB_GLOBAL
			local := ST_BIND(info) == STB_LOCAL
			if local && global {
				flags[0] = '!'
			} else if local {
				flags[0] = 'l'
		...
		weak := ST_BIND(info) == STB_WEAK
		if weak {
			flags[1] = 'w'
		}

		name := sym.Name
		if ST_TYPE(info) == STT_SECTION {
			flags[5] = 'd'
			name = section
		}

		obj := ST_TYPE(info) == STT_OBJECT
		file := ST_TYPE(info) == STT_FILE
		fn := ST_TYPE(info) == STT_FUNC
		...
		fmt.Fprintf(w, "%016x %s %s\t%016x %s%s\n", sym.Value, flags, section, sym.Size, vis, name)
	}
...
func xxd(hw io.Writer, size int64, ident [16]uint8, r io.Reader, key string) error {
	...
	case ELFCLASS32:
		...
		if err := binary.Read(sr, f.ByteOrder, hdr); err != nil {
			return fmt.Errorf("read elf32: %w", err)
		}
		...
		f.phoff = int64(hdr.Phoff)
		f.phentsize = int(hdr.Phentsize)
		f.phnum = int(hdr.Phnum)
		f.shoff = int64(hdr.Shoff)
	...
	case ELFCLASS64:
		hdr := new(Header64)
		if err := binary.Read(sr, f.ByteOrder, hdr); err != nil {
			return fmt.Errorf("read elf64: %w", err)
		}
		f.Type = Type(hdr.Type)
		f.Machine = Machine(hdr.Machine)
		f.Entry = hdr.Entry
	...
	if f.shoff < 0 {
		return &FormatError{0, "invalid shoff", f.shoff}
	}
	if f.phoff < 0 {
		return &FormatError{0, "invalid phoff", f.phoff}
	}

	if f.shoff == 0 && f.shnum != 0 {
		return &FormatError{0, "invalid ELF shnum for shoff=0", f.shnum}
	}

	if f.shnum > 0 && f.shstrndx >= f.shnum {
		return &FormatError{0, "invalid ELF shstrndx", f.shstrndx}
	}
	...
	if f.phnum > 0 && f.phentsize < wantPhentsize {
		return &FormatError{0, "invalid ELF phentsize", f.phentsize}
	}
	...
	for i := 0; i < f.phnum; i++ {
		...
		case ELFCLASS32:
			...
			if err := binary.Read(sr, f.ByteOrder, ph); err != nil {
				return fmt.Errorf("read 32 header: %w", err)
			}
		...
		case ELFCLASS64:
			ph := new(Prog64)
			if err := binary.Read(sr, f.ByteOrder, ph); err != nil {
				return fmt.Errorf("read 64 header: %w", err)
			}
			p.ProgHeader = ProgHeader{
				Type:   ProgType(ph.Type),
				Flags:  ProgFlag(ph.Flags),
		...
		if int64(p.Off) < 0 {
			return &FormatError{off, "invalid program header offset", p.Off}
		}
		if int64(p.Filesz) < 0 {
			return &FormatError{off, "invalid program header file size", p.Filesz}
		}
		f.Progs[i] = p
	}
	...
	if f.shoff > 0 && f.shnum == 0 {
		...
		case ELFCLASS32:
			...
			if err := binary.Read(sr, f.ByteOrder, sh); err != nil {
				return fmt.Errorf("sh_size 32: %w", err)
			}
			f.shnum = int(sh.Size)
			typ = sh.Type
			link = sh.Link
		case ELFCLASS64:
			sh := new(Section64)
			if err := binary.Read(sr, f.ByteOrder, sh); err != nil {
				return fmt.Errorf("sh_size 64: %w", err)
			}
			f.shnum = int(sh.Size)
			typ = sh.Type
			link = sh.Link
		...
		if SectionType(typ) != SHT_NULL {
			return &FormatError{f.shoff, "invalid type of the initial section", SectionType(typ)}
		}

		if f.shnum < int(SHN_LORESERVE) {
			return &FormatError{f.shoff, "invalid ELF shnum contained in sh_size", f.shnum}
		}
		...
		if f.shstrndx == int(SHN_XINDEX) {
			f.shstrndx = int(link)
			if f.shstrndx < int(SHN_LORESERVE) {
				return &FormatError{f.shoff, "invalid ELF shstrndx contained in sh_link", f.shstrndx}
			}
		}
	}

	if f.shnum > 0 && f.shentsize < wantShentsize {
		return &FormatError{0, "invalid ELF shentsize", f.shentsize}
	}

	// Read section headers
	c := SliceCap((*Section)(nil), uint64(f.shnum))
	if c < 0 {
		return &FormatError{0, "too many sections", f.shnum}
	}
	...
	for i := 0; i < f.shnum; i++ {
		...
		case ELFCLASS32:
			sh := new(Section32)
			if err := binary.Read(sr, f.ByteOrder, sh); err != nil {
				return fmt.Errorf("32 section[%d] read: %w", i, err)
			}
		...
		case ELFCLASS64:
			sh := new(Section64)
			if err := binary.Read(sr, f.ByteOrder, sh); err != nil {
				return fmt.Errorf("64 section[%d] read: %w", i, err)
			}
			f.names = append(f.names, sh.Name)
			s.SectionHeader = SectionHeader{
				Type:      SectionType(sh.Type),
		...
		if int64(s.FileSize) < 0 {
			return &FormatError{off, "invalid section size", int64(s.FileSize)}
		}
...
func (i Class) ElfString() string { return stringName(uint32(i), elfClassStrings, false) }
...
func (i ProgFlag) ElfString() string {
	result := []byte("---")
	for idx, pfs := range pfStrings {
		if uint32(i)&pfs.i != 0 {
			result[2-idx] = strings.ToLower(pfs.s)[3]
		}
	}
...
// seekForward converts an io.Reader into an io.ReadSeeker.
...
func (f *File) stringTable(link uint32, r io.ReadSeeker) ([]byte, error) {
...
func (f *File) getSymbols(typ SectionType, r io.ReadSeeker) ([]Symbol, []byte, error) {
	switch f.Class {
	case ELFCLASS64:
		return f.getSymbols64(typ, r)

	case ELFCLASS32:
		return f.getSymbols32(typ, r)
	}
...
func (f *File) getSymbols32(typ SectionType, r io.ReadSeeker) ([]Symbol, []byte, error) {
	...
	strdata, err := f.stringTable(symtabSection.Link, r)
	if err != nil {
...
func (f *File) getSymbols64(typ SectionType, r io.ReadSeeker) ([]Symbol, []byte, error) {
	symtabSection := f.SectionByType(typ)
	if symtabSection == nil {
		return nil, nil, ErrNoSymbols
	}
	...
	if _, err := symtab.Read(skip[:]); err != nil {
		return nil, nil, err
	}
...
func (f *File) ImportedSymbols(r io.ReadSeeker) ([]ImportedSymbol, error) {
	sym, err := f.DynamicSymbols(r)
	if err != nil {
		return nil, err
	}
...
func (f *File) gnuVersionInit(str []byte, r io.ReadSeeker) bool {
	if f.gnuNeed != nil {
		// Already initialized
		return true
	}
	...
	if vn != nil {
		...
		for {
			...
			for c := 0; c < int(cnt); c++ {
				...
				hashoff := f.ByteOrder.Uint32(d[j : j+4])
...
func (f *File) ImportedLibraries(r io.ReadSeeker) ([]string, error) {
	return f.DynString(DT_NEEDED, r)
}
...
func (f *File) DynamicSection(r io.ReadSeeker) ([]DynSec, error) {
	...
	str, err := f.stringTable(ds.Link, r)
	if err != nil {
		return nil, err
	}
	var all []DynSec
...
// DT_RUNPATH.
func (f *File) DynString(tag DynTag, r io.ReadSeeker) ([]string, error) {
	switch tag {
	case DT_NEEDED, DT_SONAME, DT_RPATH, DT_RUNPATH:
	default:
