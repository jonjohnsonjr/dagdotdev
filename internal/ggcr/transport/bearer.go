...
// NewBearer returns a bearer-token RoundTripper plus the registry's
// TokenResponse for reuse.
func NewBearer(ctx context.Context, pr *PingResp, reg name.Registry, auth authn.Authenticator, t http.RoundTripper, scopes []string) (http.RoundTripper, *TokenResponse, error) {
	switch pr.challenge.Canonical() {
	case anonymous, basic:
		return &basicTransport{inner: t, auth: auth, target: reg.RegistryStr()}, nil, nil
	}
...
// OldBearer returns a bearer-token RoundTripper from a cached TokenResponse.
func OldBearer(pr *PingResp, tok *TokenResponse, reg name.Registry, auth authn.Authenticator, t http.RoundTripper, scopes []string) (http.RoundTripper, error) {
	switch pr.challenge.Canonical() {
	case anonymous, basic:
...
func (bt *bearerTransport) RoundTrip(in *http.Request) (*http.Response, error) {
	...
	if challenges := ResponseChallenges(res); len(challenges) != 0 {
