...
import (
	...
	"github.com/jonjohnsonjr/dagdotdev/internal/ggcr/internal/redact"
	"github.com/jonjohnsonjr/dagdotdev/internal/ggcr/internal/verify"
...
// Layer is the read handle returned by Layer() / Blob fetching.
//
// External callers only invoke Compressed() and Size(); the field-level
// metadata (digest, ref) is kept around because the BlobSeeker code below
// needs it.
type Layer struct {
	fetcher
	digest v1.Hash
...
// Compressed streams the layer blob bytes.
func (rl *Layer) Compressed() (io.ReadCloser, error) {
...
// Size returns the compressed blob size, issuing a HEAD if it's not already
// known. ECR's HEAD support is broken so we GET there.
func (rl *Layer) Size() (int64, error) {
	if rl.size > 0 {
		return rl.size, nil
	}
	var (
...
// NewLayer reads the given blob reference from a registry. A blob reference
// is a punned name.Digest where the digest portion is the digest of the blob
// to fetch and the repository portion is the repo it lives in.
func NewLayer(ref name.Digest, options ...Option) (*Layer, error) {
